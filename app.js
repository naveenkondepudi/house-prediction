...
function initPredictForm(root) {
  ...
  function getDistrictMandals(district) {
    const districtKey = canonicalDistrictName(district);
    return allMandals.filter(entry => canonicalDistrictName(entry.district) === districtKey);
  }
  ...
  async function loadVillagesForDistrict(district) {
    ...
    try {
      ...
      if (districtInput.value === district) {
        ...
        setMandalOptions(districtMandals.length ? districtMandals : villageMandals);

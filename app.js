...
function runPrediction(inputs) {
  const base = 3; // base price in lakhs
  ...
  const typeAdj = { Villa: 14, Penthouse: 18, 'Independent House': 6, Apartment: 0, Studio: -8, Land: 0 }[inputs.propertyType] || 0;
    ...
    <section class="section" style="padding-top:0;">
      <div class="section-inner">
        <div class="predict-layout">
          <div class="programme-watermark" aria-hidden="true">
            <svg viewBox="0 0 48 48"><path d="M7 22 24 8l17 14v17a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2V22Z"/><path d="M18 41V27h12v14M4 23 24 6l20 17"/></svg>
            <span>ESTATEAI</span>
          </div>
          <div class="predict-form-card glass">
            ...
            <form id="predictForm" novalidate>
              <div class="form-grid">
                <div class="form-field building-field" data-field="bedrooms">
                ...
                <div class="form-field building-field" data-field="bathrooms">
                ...
                <div class="form-field building-field" data-field="floors">
                ...
                <div class="form-field building-field" data-field="age">
                ...
                <div class="form-field building-field" data-field="garage">
                ...
                <div class="form-field building-field" data-field="condition">
                ...
                <div class="form-field" data-field="propertyType">
                  ...
                  <select id="f_propertyType">
                    <option>Apartment</option><option>Independent House</option><option>Villa</option><option>Studio</option><option>Penthouse</option><option>Land</option>
                  </select>
                </div>
                <div class="form-field" data-field="state">
                  <label>State / Union Territory</label>
                  <input type="search" class="location-search" data-location-search="f_state" placeholder="Search states..." aria-label="Search states">
                  <select id="f_state"></select>
                </div>
                <div class="form-field" data-field="district">
                  <label>District</label>
                  <input type="search" class="location-search" data-location-search="f_district" placeholder="Search districts..." aria-label="Search districts">
                  <select id="f_district"><option value="">Loading districts…</option></select>
                </div>
                <div class="form-field" data-field="mandal">
                  <label>Mandal</label>
                  <input type="search" class="location-search" data-location-search="f_mandal" placeholder="Search mandals..." aria-label="Search mandals">
                  <select id="f_mandal"><option value="">Select a district first</option></select>
                </div>
                <div class="form-field" data-field="locality">
                  <label>Village / Locality</label>
                  <input type="search" class="location-search" data-location-search="f_locality" placeholder="Search villages..." aria-label="Search villages">
                  <select id="f_locality"></select>
                </div>
                <div class="form-field" data-field="zip">
                  <label>Zip Code</label>
                  <input type="text" id="f_zip" maxlength="6" inputmode="numeric" placeholder="e.g. 500032">
...
function initPredictForm(root) {
  const stateSelect = root.querySelector('#f_state');
  const districtInput = root.querySelector('#f_district');
  const mandalInput = root.querySelector('#f_mandal');
  const zipInput = root.querySelector('#f_zip');
  let districtsByState = {};
  let placesByDistrict = {};
  let villagesByMandal = {};
  let allMandals = [];
  let pendingMandal = null;
  stateSelect.innerHTML = INDIAN_STATES.map(state => `<option value="${state}">${state}</option>`).join('');

  function applyLocationSearch(select) {
    const search = root.querySelector(`[data-location-search="${select.id}"]`);
  ...
  ['f_state', 'f_district', 'f_mandal', 'f_locality'].forEach(selectId => {
    const select = root.querySelector(`#${selectId}`);
    const search = root.querySelector(`[data-location-search="${selectId}"]`);
    search.addEventListener('input', () => applyLocationSearch(select));
  });
  applyLocationSearch(stateSelect);

  function refreshLocalities(city) {
    const localitySelect = root.querySelector('#f_locality');
    const list = getDistrictPlaces(city);
    localitySelect.innerHTML = list.map(l => `<option value="${l}">${l}</option>`).join('');
    applyLocationSearch(localitySelect);
  }

  const propertyTypeSelect = root.querySelector('#f_propertyType');
  function syncPropertyTypeFields() {
    const isLand = propertyTypeSelect.value === 'Land';
    root.querySelectorAll('.building-field').forEach(field => { field.hidden = isLand; });
  }
  propertyTypeSelect.addEventListener('change', syncPropertyTypeFields);
  stateSelect.addEventListener('change', () => {
    populateDistricts(stateSelect.value);
    setMandalOptions([]);
    setLocalityOptions([]);
    loadVillagesForDistrict(districtInput.value);
  });
  districtInput.addEventListener('change', () => {
    const districtPlaces = getDistrictPlaces(districtInput.value);
    setMandalOptions(getDistrictMandals(districtInput.value));
    setLocalityOptions(districtPlaces);
    loadVillagesForDistrict(districtInput.value);
  });
  mandalInput.addEventListener('change', () => {
    const [mandalState, district, mandal] = mandalInput.value.split('::');
    pendingMandal = mandal ? { state: mandalState, district, mandal } : null;
    if (mandalState && stateSelect.value !== mandalState) {
      stateSelect.value = mandalState;
      populateDistricts(mandalState);
    }
    if (district && districtInput.value !== district) {
      const districtOption = [...districtInput.options].find(option => option.value === district);
      if (districtOption) districtInput.value = district;
      loadVillagesForDistrict(district);
    }
    setLocalityOptions(mandal ? villagesByMandal[mandal] || [] : getAllVillages());
  });

  function populateDistricts(state, selectedDistrict = '') {
    const districts = [...(districtsByState[state] || [])];
    if (selectedDistrict && !districts.includes(selectedDistrict)) districts.unshift(selectedDistrict);
    districtInput.innerHTML = districts.length
      ? districts.map(district => `<option value="${district}">${district}</option>`).join('')
      : '<option value="">Verify a pincode to load the district</option>';
    if (selectedDistrict && districts.includes(selectedDistrict)) districtInput.value = selectedDistrict;
    applyLocationSearch(districtInput);
  }

  function normalizeLocationName(name) {
    return String(name || '').toLowerCase().replace(/district|\.|\s+/g, '').replace(/[aeiou]/g, '');
  }

  function getDistrictPlaces(district) {
    const directoryPlaces = placesByDistrict[normalizeLocationName(district)] || [];
    return [...new Set([...directoryPlaces, ...(LOCALITIES[district] || [])])];
  }

  function setMandalOptions(mandals) {
    const options = mandals.map(entry => {
      const district = typeof entry === 'string' ? '' : entry.district;
      const mandal = typeof entry === 'string' ? entry : entry.mandal;
      const state = typeof entry === 'string' ? '' : entry.state;
      return `<option value="${district ? `${state}::${district}::${mandal}` : mandal}">${district ? `${district} — ` : ''}${mandal}</option>`;
    }).join('');
    mandalInput.innerHTML = mandals.length
      ? `<option value="">All Mandals</option>${options}`
      : '<option value="">No mandals loaded</option>';
    applyLocationSearch(mandalInput);
  }

  function getDistrictMandals(district) {
    const districtKey = canonicalDistrictName(getMandalSourceDistrict(district));
    const stateKey = canonicalStateName(stateSelect.value);
    const exactMatches = allMandals
      .filter(entry => canonicalStateName(entry.state) === stateKey && canonicalDistrictName(entry.district) === districtKey)
    const matches = exactMatches.length
      ? exactMatches
      : allMandals.filter(entry => canonicalStateName(entry.state) === stateKey
        && normalizeLocationName(entry.district) === normalizeLocationName(getMandalSourceDistrict(district)));
    return matches.map(entry => ({ ...entry, district }));
  }

  function canonicalStateName(name) {
    const key = String(name || '').toLowerCase().replace(/unionterritory|\.|\s+|&/g, '');
    const aliases = {
      andamanandnicobarislands: 'andamannicobarislands',
      dadranagarhavelianddamananddiu: 'dadranagarhaveli',
  ...
  function canonicalDistrictName(name) {
    const key = String(name || '').toLowerCase().replace(/district|\.|\s+/g, '');
    const aliases = {
      ananthapuramu: 'anantapur',
      anantapuramu: 'anantapur',
      srisathyasai: 'srisathyasai',
      ysrkadapa: 'kadapa',
      cuddapah: 'kadapa',
  ...
  function getMandalSourceDistrict(district) {
    return DISTRICT_MANDAL_SOURCE_ALIASES[canonicalDistrictName(district)] || district;
  }

  function getAllVillages() {
    return [...new Set(Object.values(villagesByMandal).flat())];
  }
  ...
  async function loadPlaceDirectory() {
    try {
      const response = await fetch(PLACE_DIRECTORY_URL);
      if (!response.ok) throw new Error('Place directory unavailable');
      const lines = (await response.text()).split(/\r?\n/).filter(Boolean).slice(1);
      placesByDistrict = {};
      lines.forEach(line => {
        const [, district, place] = parseCsvLine(line);
        if (!district || !place) return;
        const key = normalizeLocationName(district);
        if (!placesByDistrict[key]) placesByDistrict[key] = [];
        if (!placesByDistrict[key].includes(place)) placesByDistrict[key].push(place);
        allMandals.push({ state: parseCsvLine(line)[0], district, mandal: place });
      });
      allMandals = allMandals.filter((entry, index, entries) => entries.findIndex(item => item.state === entry.state && item.district === entry.district && item.mandal === entry.mandal) === index);
      setLocalityOptions(getDistrictPlaces(districtInput.value));
      setMandalOptions(getDistrictMandals(districtInput.value));
      loadVillagesForDistrict(districtInput.value);
    } catch (error) {
      setLocalityOptions(getDistrictPlaces(districtInput.value));
  ...
  async function loadVillagesForDistrict(district) {
    if (!district) return;
    const districtKey = normalizeLocationName(district);
    const alias = Object.entries(VILLAGE_FILE_ALIASES)
      .find(([name]) => normalizeLocationName(name) === districtKey)?.[1];
    const fileName = alias
      || `${district.toUpperCase()}.json`;
    try {
      const response = await fetch(`${VILLAGE_DIRECTORY_BASE_URL}${encodeURIComponent(fileName)}`);
      if (!response.ok) return;
      const payload = await response.json();
      villagesByMandal = Array.isArray(payload)
        ? payload.reduce((groups, entry) => {
          const mandal = entry.mandal || 'Other';
          (groups[mandal] ||= []).push(entry.village);
          return groups;
        }, {})
        : Object.fromEntries(Object.entries(payload).map(([mandal, villages]) => [mandal, [...new Set(villages.filter(Boolean))]]));
      if (districtInput.value === district) {
        const districtMandals = getDistrictMandals(district);
        const villageMandals = Object.keys(villagesByMandal).map(mandal => ({
          state: stateSelect.value,
          district,
          mandal,
        }));
        setMandalOptions(villageMandals.length ? villageMandals : districtMandals);
        if (pendingMandal?.district === district && pendingMandal.mandal) {
          mandalInput.value = `${pendingMandal.state}::${district}::${pendingMandal.mandal}`;
          setLocalityOptions(villagesByMandal[pendingMandal.mandal] || []);
          pendingMandal = null;
        } else {
          setLocalityOptions(getAllVillages());
        }
      }
    } catch (error) {
      // Keep the district place list when a village file is unavailable.
      villagesByMandal = {};
      setMandalOptions(getDistrictMandals(district));
    }
  }

  async function loadDistrictDirectory() {
    try {
      const response = await fetch(DISTRICT_DIRECTORY_URL);
      if (!response.ok) throw new Error('District directory unavailable');
      const payload = await response.json();
      districtsByState = Object.fromEntries((payload.states || []).map(entry => [entry.state, entry.districts || []]));
      districtsByState = { ...districtsByState, ...DISTRICT_OVERRIDES };
      populateDistricts(stateSelect.value, districtInput.value || SAMPLE_PROPERTY_FORM.district);
      loadVillagesForDistrict(districtInput.value);
    } catch (error) {
      populateDistricts(stateSelect.value);
    }
  ...
  function setLocalityOptions(localities) {
    const localitySelect = root.querySelector('#f_locality');
    localitySelect.innerHTML = localities.map(locality => `<option value="${locality}">${locality}</option>`).join('');
    applyLocationSearch(localitySelect);
  }
  ...
  form.addEventListener('submit', (e) => {
    ...
    const inputs = {
      ...
      city: root.querySelector('#f_district').value,
      state: root.querySelector('#f_state').value,
      district: root.querySelector('#f_district').value,
      mandal: root.querySelector('#f_mandal').value,
    ...
    setTimeout(() => {
      ...
      const record = {
        ...
        city: inputs.city, state: inputs.state, district: inputs.district, mandal: inputs.mandal, locality: inputs.locality, propertyType: inputs.propertyType,
          ...
          <div>
            <div class="contact-info-card">
              <div class="contact-info-row">
                ...
                <div><h5>Email</h5><p>yourestate.2026@gmail.com</p></div>
              </div>
              <div class="contact-info-row">
                ...
                <div><h5>Phone</h5><p>+917095503204</p></div>
              </div>
              <div class="contact-info-row">
                ...
                <div><h5>Office</h5><p>Mummidivaram, Dr. B. R. Ambedkar Konaseema District, Andhra Pradesh, India</p></div>
              </div>
            </div>
            <div class="map-embed">
              <svg viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 1118 0z"/><circle cx="12" cy="10" r="3"/></svg>
              <span>Mummidivaram, Dr. B. R. Ambedkar Konaseema District, Andhra Pradesh, India</span>
            </div>
          </div>
